export const reviews = [
  {
    review:
      "Amazing! Angela took care of me and made sure my eyebrows were stunning",
    name: "Bella Swan",
    src: "./images/bella.jpg",
    alt: "Girl with make up and dark hair with tattoos, image from sobhan joodi on Unsplash",
    id: 0,
  },
  {
    review:
      "I'll definitely will be back! Rosalie was so nice and really knew what she was doing!   ",
    name: "Alice Cullen",
    src: "Girl with make up, short hair, and a hat, image by mahdi chaghari on Unsplash",
    id: 1,
  },
  {
    review:
      "I was in town for a wedding, and Renesmee was able to fit me in! Super fast and painless!",
    name: "Jessica Stanley",
    src: "./images/jessica.jpg",
    alt: "Girl smiling with long bronze hair in the garden, image by Ayo Ogunseinde",

    id: 2,
  },
];
